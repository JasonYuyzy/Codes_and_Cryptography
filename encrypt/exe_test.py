    ...
    for i in range(list_length1):
        for j in range(list_length2):
            for k in track(range(total)):
                ...
                if k < total/6:
                    combine = word_list1[first] + '.' + word_list2[second] + '.' + word_list3[random.randint(0, list_length3-1)]
                elif k >= total/6 and k < (total*2)/6:
                    combine = word_list3[first] + '.' + word_list1[second] + '.' + word_list2[random.randint(0, list_length3-1)]
                elif k >= (total*2)/6 and k < (total*3)/6:
                    combine = word_list3[first] + '.' + word_list2[second] + '.' + word_list1[random.randint(0, list_length3-1)]
                elif k >= (total*3)/6 and k < (total*4)/6:
                    combine = word_list2[first] + '.' + word_list1[second] + '.' + word_list3[random.randint(0, list_length3-1)]
                elif k >= (total*4)/6 and k < (total*5)/6:
                    combine = word_list1[first] + '.' + word_list3[second] + '.' + word_list2[random.randint(0, list_length3-1)]
                else:
                    combine = word_list2[first] + '.' + word_list3[second] + '.' + word_list1[random.randint(0, list_length3-1)]
                if len(combine) == 16:
